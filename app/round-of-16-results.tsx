...
import { generateRoundOf16Bracket } from '@/services/knockout-bracket';
...
export default function RoundOf16ResultsScreen() {
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      ...
      try {
        ...
        if (!bracketData) {
          bracketData = await AsyncStorage.getItem(ROUND_OF_32_BRACKET_KEY);
        }
        if (!predictionsData) {
          predictionsData = await AsyncStorage.getItem('round_of_32_predictions_for_r16');
        }
        ...
        if (bracketData && predictionsData) {
          ...
          const { getR32Predictions } = await import('@/services/async-predictions');
  ...
  const handleContinue = () => {
    ...
    AsyncStorage.setItem('round_of_16_bracket', JSON.stringify(r16Bracket));
    
    router.push({
      pathname: '/round-of-16-predictions',
      params: {
