...
export default function RoundOf16PredictionsScreen() {
    ...
    const init = async () => {
      ...
      try {
        ...
        if (bracketData) {
          ...
          const { getR16Predictions, getAntePostLockedStatus } = await import('@/services/async-predictions');
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const { saveR16Predictions } = await import('@/services/async-predictions');
  ...
  const handleContinue = async () => {
    ...
    try {
      ...
      await AsyncStorage.setItem('round_of_16_predictions_for_qf', JSON.stringify(r16Predictions));
      
      // Navigate to Quarter Finals results screen
      router.push({
        pathname: '/quarter-finals-results',
        params: {
