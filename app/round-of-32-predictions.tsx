...
export default function RoundOf32PredictionsScreen() {
    ...
    const init = async () => {
      ...
      try {
        ...
        if (bracketData) {
          ...
          const { getR32Predictions, getAntePostLockedStatus } = await import('@/services/async-predictions');
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const { saveR32Predictions } = await import('@/services/async-predictions');
  ...
  const handleContinue = async () => {
    ...
    try {
      ...
      await AsyncStorage.setItem('round_of_32_predictions_for_r16', JSON.stringify(r32Predictions));
      
      // Navigate to Round of 16 results screen
      router.push({
        pathname: '/round-of-16-results',
        params: {
