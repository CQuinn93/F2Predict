...
export default function QuarterFinalsPredictionsScreen() {
    ...
    const init = async () => {
      ...
      try {
        ...
        if (bracketData) {
          ...
          const { getQFPredictions, getAntePostLockedStatus } = await import('@/services/async-predictions');
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const { saveQFPredictions } = await import('@/services/async-predictions');
  ...
  const handleContinue = async () => {
    ...
    try {
      ...
      await AsyncStorage.setItem('quarter_finals_predictions_for_sf', JSON.stringify(qfPredictions));
      
      // Navigate to Semi Finals results screen
      router.push({
        pathname: '/semi-finals-results',
        params: {
