...
const BRONZE_FINAL_BRACKET_KEY = 'bronze_final_bracket';
...
export default function BronzeFinalPredictionsScreen() {
    ...
    const init = async () => {
      ...
      try {
        ...
        if (bracketData) {
          ...
          const { getBronzeFinalPredictions, getAntePostLockedStatus } = await import('@/services/async-predictions');
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const { saveBronzeFinalPredictions } = await import('@/services/async-predictions');
  ...
  const handleContinue = async () => {
    ...
    try {
      ...
      const sfBracketData = await AsyncStorage.getItem('semi_finals_bracket');
      if (!sfBracketData) {
      ...
      const { generateFinalBracket } = await import('@/services/knockout-bracket');
      const finalBracket = generateFinalBracket(formattedSFPredictions, sfBracket);
      
      // Store for Final predictions
      await AsyncStorage.setItem('final_bracket', JSON.stringify(finalBracket));
      
      // Navigate to Final predictions
      router.push({
        pathname: '/final-predictions',
        params: {
